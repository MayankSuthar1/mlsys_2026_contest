...
def _routing_and_dispatch(routing_logits, routing_bias, E_global, N_GROUP,
    ...
    # Reduce routed fan-out only for very long sequences when the weakest route is tiny.
    if T >= 4096:
        min_vals, min_idx = sel_weights.min(dim=1)
        drop_mask = min_vals <= 0.05
        if drop_mask.any():
            active_mask_2d = torch.ones_like(sel_weights, dtype=torch.bool)
            active_mask_2d[drop_mask, min_idx[drop_mask]] = False
